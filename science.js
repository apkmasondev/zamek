...
export function drawExperiment(canvas,id,v,t=0){
 ...
 switch(id){
 case 1:{line([[60,190],[427.6,220],[553.4,222]],white,7);ctx.beginPath();ctx.moveTo(360,350);ctx.lineTo(490,100);ctx.lineTo(620,350);ctx.closePath();ctx.fillStyle='#9ccdd022';ctx.fill();ctx.strokeStyle=cyan;ctx.lineWidth=3;ctx.stroke();
  // Unspread, the colours travel together and read as white; spread, red bends least and violet most (towards the base).
  const colors=['#f57363','#efab62','#e6d776','#87cfa2','#77aada','#b19ae2'],s=v/100,end=j=>[940,252+j*s*20];
  colors.forEach((c,j)=>line([[553.4,222],end(j)],c,5));ctx.globalAlpha=Math.max(0,1-s*5);line([[553.4,222],end(2.5)],white,7);ctx.globalAlpha=1;
  label('BIAŁE ŚWIATŁO',220,140,17);label('PRYZMAT',490,397,17);if(s>.3){label('czerwień',880,end(0)[1]-14,16,colors[0]);label('fiolet',880,end(5)[1]+26,16,colors[5]);}break;}
 ...
 case 5:{const e=energyBalance(v);ctx.fillStyle=muted;ctx.fillRect(130,150,150,130);label('100',205,222,40);arrow([300,215],[435,215]);ctx.fillStyle=gold;ctx.fillRect(450,150,420*e.useful/100,55);ctx.fillStyle=cyan;ctx.fillRect(450,235,420*e.environment/100,55);
  label(String(e.useful),480+420*e.useful/100,187,24,gold);label(String(e.environment),480+420*e.environment/100,272,24,cyan);
  label('energia użyteczna',630,130,21,gold);label('energia do otoczenia',650,328,21,cyan);label(`${e.useful} + ${e.environment} = 100`,500,400,24,white);break;}
 case 6:{const c=circuit(v);line([[230,120],[730,120],[730,325],[230,325],[230,120]],gold,5);
  // Current, drawn in its conventional direction (from + around to −) at a pace proportional to I. The pace is illustrative:
  // real charges drift far slower, which the room's fact explains.
  const loop=[[230,325],[230,120],[730,120],[730,325],[230,325]],seg=loop.slice(1).map((q,i)=>Math.hypot(q[0]-loop[i][0],q[1]-loop[i][1])),L=seg.reduce((a,b)=>a+b);
  if(c.current>0)for(let j=0;j<26;j++){let d=((j/26)*L+t*c.current*110)%L,i=0;while(d>seg[i]){d-=seg[i];i++;}const a=loop[i],b=loop[i+1],f=d/seg[i],x=a[0]+(b[0]-a[0])*f,y=a[1]+(b[1]-a[1])*f;
   if((x<270&&y>185&&y<245)||(x>680&&y>165&&y<262))continue;circle(x,y,5,'#f8d38c');}
  ctx.fillStyle='#101e25';ctx.fillRect(195,192,70,45);line([[195,202],[265,202]],gold,5);line([[211,225],[249,225]],gold,5);label('+',180,208,20,gold);label('−',180,236,20,gold);
  ctx.fillStyle='#101e25';ctx.fillRect(685,170,90,88);ctx.strokeStyle=cyan;ctx.lineWidth=3;ctx.strokeRect(708,170,44,88);if(v>0){ctx.shadowColor='#f8c772';ctx.shadowBlur=v*3;ctx.fillStyle=`rgba(238,190,110,${v/20})`;ctx.fillRect(710,172,40,84);ctx.shadowBlur=0;}
  label(`${formatPl(v,1)} V`,120,224,28);label('R = 10 Ω',850,222,26);label(`I = ${formatPl(c.current,2)} A`,500,385,25);label('IDEALNY OPORNIK · OBWÓD ZAMKNIĘTY',500,60,16,muted);
  if(c.current>0){arrow([440,100],[520,100],'#f8d38c');label('kierunek prądu · tempo umowne',480,428,15,muted);}break;}
 case 7:{const a=v*Math.PI/180,ca=Math.cos(a),sa=Math.sin(a),rot=(x,y)=>[500+x*ca-y*sa,225+x*sa+y*ca];
  ctx.save();ctx.translate(500,225);ctx.rotate(a);for(const h of [60,100,150]){ctx.beginPath();ctx.ellipse(0,0,210,h,0,0,Math.PI*2);ctx.strokeStyle=cyan;ctx.lineWidth=2;ctx.stroke();arrow([-12,-h],[12,-h],cyan);arrow([-12,h],[12,h],cyan);}
  ctx.fillStyle=gold;ctx.fillRect(-100,-25,100,50);ctx.fillStyle=muted;ctx.fillRect(0,-25,100,50);arrow([84,15],[-84,15],white);ctx.restore();
  // Letters stay upright while the magnet turns.
  const [nx,ny]=rot(-50,-4),[sx,sy]=rot(50,-4);label('N',nx,ny+9,25,'#101e25');label('S',sx,sy+9,25);
  // Compass needles around the magnet follow the field of its two poles; the red tip is the needle's north end.
  const N=rot(-80,0),S=rot(80,0);
  for(const [px,py] of [[120,80],[120,225],[120,370],[300,50],[300,400],[500,40],[500,410],[700,50],[700,400],[880,80],[880,225],[880,370]]){
   let bx=0,by=0;for(const [q,[qx,qy]] of [[1,N],[-1,S]]){const dx=px-qx,dy=py-qy,r3=Math.pow(dx*dx+dy*dy,1.5);bx+=q*dx/r3;by+=q*dy/r3;}
   const k=Math.hypot(bx,by)||1,ux=bx/k*22,uy=by/k*22;line([[px,py],[px+ux,py+uy]],'#e0604a',5);line([[px,py],[px-ux,py-uy]],'#d8d2c4',5);circle(px,py,3.5,'#101e25');}
  label('igła kompasu: czerwony koniec wskazuje kierunek pola',500,440,15,muted);break;}
 case 8:{const phase=Math.round(v);for(let j=0;j<36;j++){let x,y;if(phase===0){x=330+(j%6)*60+Math.sin(t*6+j)*3;y=90+Math.floor(j/6)*50+Math.cos(t*6+j)*3;}else if(phase===1){x=280+((j*77.3+Math.sin(t*.7+j)*30)%420+420)%420;y=230+((j*34.6+Math.cos(t*.6+j)*25)%110+110)%110;}else{x=80+((j*89.3+t*(30+(j%5)*10)*(j%2?1:-1))%840+840)%840;y=60+((j*53.6+t*(20+(j%7)*4)*(j%3?1:-1))%300+300)%300;}circle(x,y,9,cyan);}label(states[phase].toUpperCase(),500,420,23);break;}
